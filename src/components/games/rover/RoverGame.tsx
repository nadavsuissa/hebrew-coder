...
import { RoverGameConfig, GameCompletionCallback } from '@/types/games';
import { CodeEditor } from '@/components/CodeEditor';
import { GameView } from '@/components/game/GameView';
import { Trophy, Target, Sparkles, BookOpen, Rocket, ChevronLeft } from 'lucide-react';
import clsx from 'clsx';
import Link from 'next/link';

interface RoverGameProps {
  config: RoverGameConfig;
  onComplete: GameCompletionCallback;
  backUrl?: string;
}

/**
 * Rover Game Component
 * Python code + rover simulation game
 * Integrates with the existing game store and code editor
 */
export function RoverGame({ config, onComplete, backUrl }: RoverGameProps) {
  const { setLevel, currentLevel, trace, currentStep } = useGameStore();
  const hasCompletedRef = useRef(false);

  // Convert config to Level format for game store
  useEffect(() => {
    const level = {
      id: 0, // Temporary ID
      title: config.title,
  ...
  }, [isWin, currentLevel, onComplete]);

  if (!currentLevel) {
    return (
      <div className="h-screen bg-[#0B1120] text-white flex items-center justify-center font-mono">
        INITIALIZING...
      </div>
    );
  }
        ...
        <div className="col-span-5 flex flex-col gap-6 h-full min-h-0">
            ...
            <div className="flex items-center justify-between shrink-0 px-2">
                ...
                <div className="flex items-center gap-3 bg-slate-900/80 px-4 py-1.5 rounded-full border border-slate-700/50 backdrop-blur">
                    ...
                    <span className="text-xs font-mono text-blue-200 tracking-widest">{currentLevel.title.toUpperCase()}</span>
                </div>
            </div>
